...
A grace period guards startup: a row spawned less than `heartbeatGraceMs` ago (default 60000) is skipped, because a container that has not finished booting has not written its first heartbeat yet and is not wedged. Without this the monitor would kill every agent moments after spawning it.

On either finding the monitor calls `registry.setStatus(threadKey, 'stopped', { containerId: null, containerName: null })` and publishes `agent_died` with the cause. It never spawns. Respawn is the sweeper's job, so that the decision to bring an agent back lives in exactly one place.
...
**A workspace whose thread has a running container is never evicted.** Deleting the directory out from under a live agent would corrupt the conversation it is mid-way through writing. Those are skipped. If skipping them means the cap still cannot be met, the GC logs a warning naming the shortfall and stops. It never deletes a protected workspace to satisfy a number.
